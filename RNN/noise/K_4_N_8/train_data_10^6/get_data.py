...
for ratio in train_ratio:
    ...
    for snr in train_snr:
        ...
        for i in range(num_train):
            ...
            x_train[i] = add_noise(bpsk(x_train_selected[random_index2]), snr)
            y_train[i] = y_train_selected[random_index2]
            
        filename = 'ratio_' + str(ratio) + '_train_snr_' + str(snr) + 'dB'
...
for snr in test_snr:
    ...
    for i in range(num_test):
        ...
        x_test[i] = add_noise(bpsk(x_data[random_index3]), snr)
        y_test[i] = d[random_index3]

    filename = 'test_snr_' + str(snr) + 'dB'
